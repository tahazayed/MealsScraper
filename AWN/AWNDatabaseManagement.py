...
##    short description of the content of WN
    ...
    def update_link(self,itemid1,itemid2,type):
        if itemid1 not in self._items:
            print ('itemid1 '+itemid1+' not present, ignored')
        elif itemid2 not in self._items:
    ...
    def update_word(self, wordid, value, synsetid):
        if synsetid not in self._items:
        ...
        for item in root.findall('item'):
            ...
            for i in wn._source_counts.keys():
                if 'item' == i:
                    wn._source_counts[i]+=1
                    break
            wn.update_item(item.get('itemid'), item.get('offset'), item.get('name'),item.get('type'), item.get('POS'))
        #<link>
        for item in root.findall('link'):
            ...
            for i in wn._source_counts.keys():
                if 'link' == i:
                    wn._source_counts[i]+=1
                    break
            wn.update_link(item.get('link1'),item.get('link2'),item.get('type'))
        #<word>
        for item in root.findall('word'):
            ...
            for i in wn._source_counts.keys():
                if 'word' == i:
                    wn._source_counts[i]+=1
                    break
            wn.update_word(item.get('wordid'),item.get('value'),item.get('synsetid'))
        #<form>
        for item in root.findall('form'):
            wn._source_counts['all']+=1
            for i in wn._source_counts.keys():
                if 'form' == i:
                    wn._source_counts[i]+=1
                    break
            wn.update_form(item.get('value'),item.get('wordid'),item.get('type'))
        #ent = open(ent)
        #print (ent)
        #wn=WN()
        #p.ParseFile(ent)
