...
class ArabicIngredientsStemmer:
    ...
    def machIngredItmPrice(self, ingrdsCorpus, ItemCorpusInp = None):
        ### mach Ingredients with Item price

        if ItemCorpusInp != None and len(ItemCorpusInp)>0 :
            ItemCorpusInp = ItemCorpusInp
        else:
            global ItemCorpus
            ItemCorpusInp = ItemCorpus
 
        names = []
        found = []
        for i in ingrdsCorpus:
            if not i in ItemCorpusInp:
                names.append(i) 
            else:
                found.append(i)
